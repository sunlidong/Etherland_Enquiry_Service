...
(function($) {
	...
	$.fn.goFaq = function(options) {
		...
		function generateSearchForm () {
			...
			var $input = $('<input>', { 'class':'col-md-8','type': 'text', 'name': 'search', 'placeholder': '请输入要查询的地址' });
			var $buton = $('<btton>',{'class':'col-md-4','type':'button','class':'btn btn-default','text':'查询'});
			<!-- 标准的按钮 -->
				...
				if(data.state==2){
					...
					for (i=0;i<data.row.length;i++){
					 ...
					 isOne.push(blockHash);
						var str ="<li><strong>交易时间:</strong>"+sTime+"<strong>交易信息:</strong>"+sFrom+"转给"+sTo+"共"+sValue+"个DRCC</li>";
						list=str+list;
						// arr.push(str);
					}
					//add
